# Time-Series-Analysis-with-Indoor-Air-Quality-by-LSTM
LONG SHORT-TERM MEMORY : The framework uses a pure LSTM model to forecast indoor $\text{CO}_2$ levels from preprocessed time-series data ($R^2 > 0.99$ for 15-min horizons). It includes SHAP, Integrated Gradients, MC Dropout, and Ensembles for explainability and uncertainty.

The repository contains reusable PyTorch pipelines for LSTM plus a broad collection of Jupyter experiments for wavelet-scattering, explainability, and uncertainty-quantification methods.

## Project capabilities

- Multi-station time-series preprocessing and chronological train/validation/test splitting
- LSTM forecasting pipelines
- Variants for CO2, AQI, and temperature prediction
- LSTM experiments for horizons from 15 minutes to 24 hours
- Optional wavelet-scattering features using Kymatio
- Optional temporal attention in the LSTM models
...
Files containing `_old` are retained legacy pipelines. For current LSTM use the `_co2.py` entry points.
...
```bash
cd "/Volumes/Hackintosh - Data/Users/Kenil_Dhorajiya/TSA"
...
- step-by-step LSTM development
- LSTM forecasting and uncertainty experiments
...
At the top of `dataPipeline_LSTM_co2.py` switches control additional work:
...
LSTM runs create a uniquely named directory in the project root. The name records major hyperparameters such as input/output length, batch size, epochs, learning rate, hidden size, resampling interval, dropout, gap handling, station encoding, scattering, and attention.
